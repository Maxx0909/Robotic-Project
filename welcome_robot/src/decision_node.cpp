...
public:
    ...
    void update()
    {
        if (init_localization)
        {
            ...
            // If robair is too far from its base (ie, its distance to the base is higher than max_base_distance),
            // then robair should stop to interact with the moving person and go back to its base.
    ...
    void process_moving_to_the_person()
    {
        ...
        if (new_person_position)
        {
            ...
            if (distancePoints(person_position, current_position) < detection_threshold && frequency >= frequency_expected)
            {
                ...
                // publish a point to zero to force stop the robot
